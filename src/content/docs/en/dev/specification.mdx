...
Note that there are no changes required to how the Stacks blockchain works today. Instead, consider these as convenience methods for wrapping existing Stacks transaction information making them easily sharable.

Stacks Pay has been submitted for consideration as a Stacks Improvement Proposal.  Comments, suggestions, and questions can be made here: [Stacks SIPs](https://github.com/stacksgov/sips)
...
Stacks Pay does *not* define smart contracts that may be used to facilitate specific functions, like subscriptions or NFT mints.  Stacks Pay is strictly focused on defining the transaction payment information structure and encoding.
...
- amount, contractName, functionName, dueDate, invoiceNumber. Wallets MUST ignore these parameters if present.
...
- operation: 'mint'
- contractAddress: A valid SIP-009 contract address.
- functionName: MUST be 'mint' 
...
- contractName, dueDate: Wallets MUST ignore these parameters if present.
...
Note: Contract addresses should be in the format `CONTRACT_PRINCIPLE`.`CONTRACT_NAME`, without the associated contract identifier.


### URL Scheme

The Stacks Pay URL scheme **MUST** use the custom protocol `web+stxpay://`, followed by an `operation` and query parameters encoding the payment details.

**Format:**

```
web+stxpay://<operation>?recipient=<recipient>&token=<token>&amount=<amount>&description=<description>[&additional_params]
```

```
web+stxpay://invoice?recipient=SP3FBR...&token=STX&amount=1000&description=Payment%20for%20Services
```
...
```
stxpay://pay?recipient=SP3FBR...&token=STX&description=Payment%20for%20Services...&amount=1000
...
- **`'support'`**: An open ended request for payment. Typically a donation or tip. 
- **`'invoice'`**: Represents a payment request for an invoice.
- **`'mint'`**: Initiates a mint operation for an nft contract.
- **`'custom-'`**: Supports open ended smart contract calls.
